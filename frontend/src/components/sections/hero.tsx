  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        ...
        <div className="absolute -bottom-[10%] left-[20%] w-[45%] h-[45%] bg-aegis-indigo/5 rounded-full blur-[140px]" aria-hidden="true" />
      </div>
        ...
        <div className="grid items-center gap-14 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          ...
          >
            {isConnected ? (
              <Link
                href="/stake"
                aria-label="Open the staking page"
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-aegis-blue to-aegis-purple opacity-0 group-hover:opacity-100 transition-opacity duration-500" aria-hidden="true" />
              </Link>
            ) : (
              <button
                type="button"
          ...
          >
            {proofPoints.map((point) => (
              <div
                key={point}
                className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/40 px-4 py-2 text-sm text-muted-foreground backdrop-blur-sm" role="listitem"
              >
                <ShieldCheck className="h-4 w-4 text-aegis-blue" aria-hidden="true" />
                <span>{point}</span>
              </div>
            ))}
          </motion.div>
          ...
          {/* Stats Grid */}
          <div className="mt-20 grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5" role="list">
            {stats.map((stat, i) => (
              <motion.div
                key={stat.label}
              ...
              >
                <stat.icon className={cn("w-5 h-5 mb-4 opacity-70 group-hover:opacity-100 transition-opacity", stat.color)} aria-hidden="true" />
